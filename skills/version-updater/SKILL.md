...
>
> **해결**: 업데이트 후 sax-architecture-checker를 **기본 모드**로 호출하여 누락 심링크 자동 생성
...
1. 서브모듈 업데이트 (git fetch + reset)
   ↓
2. sax-architecture-checker 기본 모드 호출 (--check-only 아님!)
   - 깨진 심링크 수정
   - 🔴 누락된 심링크 자동 생성 (NEW)
   ↓
...
**sax-architecture-checker 기본 모드가 수행하는 작업**:

| 검사 항목 | 수정 동작 |
|-----------|----------|
| 깨진 심링크 | 재생성 |
| 누락된 심링크 | **신규 생성** (sax-core/sax-pkg 비교) |
| .merged 마커 누락 | 생성 |
