...
LOCAL_PO=$(cat .claude/sax-po/VERSION 2>/dev/null || cat .claude/VERSION)

# 원격 버전 (GitHub API)
REMOTE_CORE=$(gh api repos/semicolon-devteam/sax-core/contents/VERSION --jq '.content' | base64 -d)
REMOTE_PO=$(gh api repos/semicolon-devteam/sax-po/contents/VERSION --jq '.content' | base64 -d)
```
