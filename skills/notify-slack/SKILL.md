---
name: notify-slack
description: Slack 채널에 메시지 전송. Use when (1) draft-task-creator 완료 후, (2) /SAX:slack 커맨드, (3) 팀원 멘션 요청, (4) 이슈/PR 공유 요청.
tools: [Bash, Read]
model: inherit
---
...
> Slack 채널에 메시지 전송 (Draft Task 알림, 커스텀 메시지, 이슈 공유 등)
>
...
| 호출 시점 | 호출 조건 |
|-----------|-----------|
| draft-task-creator 완료 후 | Draft Task 생성 완료 시 자동 호출 |
...
```text
# 자동 호출
draft-task-creator → Draft Tasks 생성 완료 → notify-slack Skill 자동 호출
...
| 유형 | 설명 | 트리거 |
|------|------|--------|
| **Draft Task 알림** | Epic/Task 생성 완료 알림 | draft-task-creator 완료 |
...
draft-task-creator로부터 다음 정보를 받습니다:

```yaml
epic:
  number: 123
  title: "댓글 기능 개선"
  url: "https://github.com/semicolon-devteam/docs/issues/123"
  project: "오피스"

tasks:
  - repo: "core-backend"
    number: 456
    title: "[Backend] 댓글 CRUD API 구현"
    assignee: "developer-github-id"
    points: 8
  - repo: "cm-introduction-new"
    number: 789
    title: "[Frontend] 댓글 UI 컴포넌트"
...
```json
{
  "blocks": [
    ...
    },
    {
      ...
      "text": {
        "type": "mrkdwn",
        "text": "• <https://github.com/semicolon-devteam/core-backend/issues/456|#456 댓글 CRUD API 구현> - <@U0XXXXXXX> (8 Points)"
      }
    },
    {
      "type": "section",
      "text": {
        "type": "mrkdwn",
        "text": "• <https://github.com/semicolon-devteam/cm-introduction-new/issues/789|#789 댓글 UI 컴포넌트> - <@U0YYYYYYY> (10 Points)"
      }
    },
    {
      "type": "context",
...
매핑 추가는 `sax-po/skills/notify-slack/references/slack-id-mapping.md` 수정
```
...
```markdown
[SAX] Skill: notify-slack 호출 - Draft Task 알림
