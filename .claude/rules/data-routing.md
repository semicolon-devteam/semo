...
| 의사결정 기록                                   | KB: `semo kb upsert {domain} decision {slug} --content "..." --decided-by {who}` (decided_at 기본값 = 오늘)                    |
| 인시던트/장애 기록                              | KB: `semo kb upsert {service} incident {slug} --content "..." --severity {level} --status {state}` (occurred_at 기본값 = 오늘) |
| 진행 상태 요약                                  | (자동) PM 파이프라인 → KB projection                                                                                           |

### `decision` 키 메타데이터 헬퍼 (CLI v4.18.9+)

`decision` 키는 `decided_by` + `decided_at`을 필수로 요구한다. JSON 메타데이터를 매번 작성하지 않도록 전용 플래그가 있다:

```bash
# 1) 플래그 직접 지정
semo kb upsert {domain} decision {slug} --content "..." --decided-by reus --decided-at 2026-04-26

# 2) 환경 변수 fallback (봇 세션에 한 번 export)
export SEMO_DECIDED_BY=semiclaw
semo kb upsert {domain} decision {slug} --content "..."   # decided_at 자동 = 오늘

# 3) 기존 방식도 유지
semo kb upsert {domain} decision {slug} --content "..." --metadata '{"decided_by":"reus","decided_at":"2026-04-26"}'
```

우선순위: `--decided-by` 플래그 > `SEMO_DECIDED_BY` env > `--metadata` 안의 `decided_by`. `decided_at` 미지정 시 오늘(YYYY-MM-DD).

> **봇 세션에서**: 자기 botId를 안다면 `--decided-by {botId}` 명시 권장. 사용자 위임 작업이면 사용자 닉네임을 그대로 사용 (예: `--decided-by reus`).

### `incident` 키 메타데이터 헬퍼 (CLI v4.18.11+)

`incident` 키는 `occurred_at` + `severity` + `status`를 필수로 요구한다:

```bash
# 1) 플래그 직접 지정 (가장 권장)
semo kb upsert {service} incident {slug} --content "..." --severity high --status investigating
# occurred_at 자동 = 오늘

# 2) 과거 인시던트 백필
semo kb upsert {service} incident {slug} --content "..." --occurred-at 2026-04-20 --severity critical --status resolved

# 3) env fallback (severity만)
export SEMO_INCIDENT_SEVERITY=medium
semo kb upsert {service} incident {slug} --content "..." --status open
```

우선순위(severity): `--severity` 플래그 > `SEMO_INCIDENT_SEVERITY` env > `--metadata` 안의 `severity`. `occurred_at` 미지정 시 오늘.
