---
name: onboarding
description: 신규 인프라 엔지니어 온보딩 프로세스 시작
---

# /SAX:onboarding Command

신규 인프라 엔지니어를 위한 온보딩 프로세스를 시작합니다.
...
3. SAX 개념 학습 (인프라 워크플로우)
4. 실습 (Docker Compose → Nginx → 배포)
5. 온보딩 완료 및 메타데이터 저장
```
...
=== SAX-Infra 온보딩 프로세스 시작 ===

Phase 0: 환경 진단
...

Phase 2: 인프라 워크플로우
1. Docker Compose 설정 (skill:scaffold-compose)
2. Nginx 설정 (skill:scaffold-nginx)
3. 환경 변수 동기화 (skill:sync-env)
4. 서비스 배포 ("/SAX:deploy")
5. 모니터링 및 롤백 ("/SAX:rollback")
```
