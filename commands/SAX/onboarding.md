---
name: onboarding
description: 신규 QA/테스터 온보딩 프로세스 시작
---

# /SAX:onboarding Command

신규 QA/테스터를 위한 온보딩 프로세스를 시작합니다.

## Trigger

- `/SAX:onboarding` 명령어
- "처음이에요", "신규", "온보딩 시작" 키워드

## Action

`onboarding-master` Agent를 호출하여 5단계 온보딩 프로세스를 안내합니다.

## Process

```text
1. 환경 진단 (skill:health-check)
2. 조직 참여 확인 (Slack, GitHub)
3. SAX 개념 학습 (QA 워크플로우)
4. 실습 (테스트 케이스 검증 → 테스트 실행)
5. 온보딩 완료 및 메타데이터 저장
```

## Expected Output

```markdown
[SAX] Orchestrator: 의도 분석 완료 → 온보딩 요청

[SAX] Agent: onboarding-master 호출 (트리거: /SAX:onboarding)

=== SAX-QA 온보딩 프로세스 시작 ===

Phase 0: 환경 진단
...

Phase 2: QA 워크플로우
1. 테스트 대기 목록 확인 ("현재 업무 확인")
2. 테스트 케이스 검증 ("/SAX:run-test #123")
3. 테스트 결과 보고 ("/SAX:test-pass #123" 또는 "/SAX:test-fail #123")
4. 버그 리포트 ("버그 리포트 작성해줘")
```

## Related

- [onboarding-master Agent](../../agents/onboarding-master/onboarding-master.md)
- [health-check Skill](../../skills/health-check/SKILL.md)
- [qa-master Agent](../../agents/qa-master/qa-master.md)
