# semo — Codex Configuration

> SEMO v4.8.0 (2026-04-15) — services/service\_\* 13개 테이블 KB 이식 완료
> 공통 규칙(KB-First, 3자 동기화, KB CLI)은 `~/.Codex/AGENTS.md` 참조.
...
`dev` push 는 dashboard Docker 빌드만 돌린다. CLI 배포 절차는 `.Codex/rules/quality-gate.md` 참조.
...
- SOUL.md, AGENTS.md의 봇 테이블은 `generateSoulMd()` 등이 DB에서 동적 생성
- 에이전트 정의(`~/.Codex/agents/`)는 DB `agent_definitions`에서 동기화
...
PM 데이터 읽기/쓰기 시 → `.Codex/rules/data-routing.md` (Data Routing)
빌드/배포 상세 → `.Codex/rules/quality-gate.md` (CLI 배포 포함)
세션 동기화 → `.Codex/rules/session-sync.md` (로컬↔Agent SDK 동기화)
