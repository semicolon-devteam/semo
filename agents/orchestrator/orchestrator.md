...
name: orchestrator
description: |
  SAX-QA orchestrator for QA testers. PROACTIVELY delegate on ALL user requests.
...
| 테스트 실패 | `skill:report-test-result` | "실패", "Fail", "/SAX:test-fail" |
...
1. **테스트 대기열 확인**: "/SAX:test-queue" 또는 "테스트할 이슈 뭐야"
2. **STG 환경 확인**: "STG 환경 확인해줘"
3. **테스트 실행**: "{repo}#{number} 테스트해줘"
4. **결과 처리**:
   - Pass: "/SAX:test-pass {repo}#{number}"
   - Fail: "/SAX:test-fail {repo}#{number} 사유: {reason}"
