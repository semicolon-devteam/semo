...
| ------------------- | ----------------------- | ---------------------------------------------- |
| **이슈 작업 시작**  | `advisor` (작업 방식 선택) | `#\d+` + (작업, 시작, 해보자, 하자, 진행, 개발), "이슈 작업", "작업 시작", "시작하자", "할당받았", "이거 해야", "뭐부터", "어떻게 시작" |
| **이슈 상태 변경**  | `skill:project-board`   | "상태 변경", "리뷰요청으로", "작업중으로", "테스트중으로", "검수완료로", "병합됨으로", "상태로 만들어", "상태로 변경", "상태로 바꿔", "이슈를 테스트중", "이슈를 리뷰요청", "이슈를 작업중", "이슈 상태" |
