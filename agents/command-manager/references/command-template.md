...
```
sax/packages/sax-po/commands/SAX/{command-name}.md
```

**예시**:
- `commands/SAX/onboarding.md` → `/SAX:onboarding`
- `commands/SAX/health-check.md` → `/SAX:health-check`
- `commands/SAX/help.md` → `/SAX:help`

### SAX-Meta 커맨드

```
sax/packages/sax-meta/commands/SAX/{command-name}.md
```
...
| 파일명 | 호출 형식 | 설명 |
|--------|-----------|------|
| `onboarding.md` | `/SAX:onboarding` | 단순 단어 |
| `health-check.md` | `/SAX:health-check` | kebab-case |
| `task-progress.md` | `/SAX:task-progress` | kebab-case |

### ❌ 잘못된 파일명

| 파일명 | 결과 | 이유 |
|--------|------|------|
| `:onboarding.md` | `/SAX::onboarding` | 이중 콜론 발생 |
| `SAX:onboarding.md` | `/SAX:SAX:onboarding` | 중복 프리픽스 |
| `OnBoarding.md` | `/SAX:OnBoarding` | PascalCase 지양 |
...
Claude: [SAX] Agent: onboarding-master 호출
