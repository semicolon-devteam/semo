...
| 의도 키워드 | 대상 Agent | 역할 |
|------------|-----------|------|
| Agent + (생성/수정/삭제/검토) | `agent-manager` | SAX Agent 라이프사이클 관리 |
| Skill + (생성/수정/삭제/검토) | `skill-manager` | SAX Skill 라이프사이클 관리 |
| Command + (생성/수정/삭제/검토) | `command-manager` | SAX Command 라이프사이클 관리 |
| 구조, 설계, 아키텍처 | `sax-architect` | SAX 패키지 설계 |
...
| 도움말, help | `sax-help` | Next.js 개발자용 가이드 |
...
| 존재하지 않는 대상 | 없는 Agent/Skill 호출 | SAX-PO에서 implementer 호출 |
