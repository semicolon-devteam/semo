...
|--------|-----------|-----------|
| sax-meta | orchestrator, agent-manager, skill-manager, command-manager, sax-architect, compliance-checker | package-validator, version-manager, package-sync, package-deploy, sax-help, feedback |
| sax-po | orchestrator, epic-master, spec-writer, task-manager, teacher | sax-help, feedback, health-check |
| sax-next | orchestrator, implementer, verifier, teacher | spec, implement, verify, task-progress, sax-help, feedback, health-check |

**위반 예시**:
- SAX-PO에서 `implementer` Agent 호출 시도
- SAX-Meta에서 `epic-master` Agent 호출 시도
...
2. sax-core 문서에서 **동일/유사 규칙** 검색
3. 중복 발견 시 **SoT 원칙** 위반으로 판정

**검색 패턴**:
```bash
# 규칙 키워드 검색
grep -r "규칙|원칙|policy|rule" sax-core/

# 유사 문장 검색
grep -r "{rule_keyword}" sax-core/ agents/ skills/
```
