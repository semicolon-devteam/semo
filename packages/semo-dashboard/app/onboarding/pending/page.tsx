    ...
    intervalRef.current = setInterval(async () => {
      try {
        ...
        if (data.profile?.onboarding_status === 'approved') {
          ...
          // 거절됨 → full reload로 AuthProvider state 초기화 후 재신청
    ...
    <div className="flex min-h-screen items-center justify-center p-4">
      <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm dark:border-gray-700 dark:bg-gray-800">
        ...
        <div className="mb-6 rounded-lg bg-gray-50 p-4 dark:bg-gray-700/50">
          <div className="mb-1 text-xs text-gray-500 dark:text-gray-400">신청 정보</div>
        ...
        <button
          onClick={signOut}
          className="rounded-lg border border-gray-300 px-6 py-2 text-sm text-gray-700 transition hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700"
        >
          로그아웃
        </button>
      </div>
