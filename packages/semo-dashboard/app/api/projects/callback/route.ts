 ...
 * Bot Callback API
 *
 * 봇(PlanClaw, GrowthClaw, DesignClaw 등)이 작업 결과를 DB에 반영하는 엔드포인트.
 * 세 가지 콜백 타입:
  ...
  try {
    ...
    switch (body.type) {
      case 'section-regeneration': {
        ...
        console.log(`[Callback] Section ${body.section_id} regenerated by ${body.bot_id}`);
      ...
      case 'stitch-export': {
        ...
        if (stitchProject) {
          ...
          if (stitchSlackCtx.channelId) {
            const screenshotUrl = body.screenshot_base64
              ? `${DASHBOARD_BASE_URL}/api/projects/${body.service_id}/stitch-screenshot/${material.material_id}`
              : undefined;
            sendServiceStitchResultSlack({
        ...
        console.log(
          `[Callback] Stitch export saved: material=${material.material_id}, section=${resultSection.section_id} by ${body.bot_id}`,
        );
      ...
      case 'design-system-confirmed': {
        ...
        console.log(
          `[Callback] Design system confirmed: stitch_project=${body.stitch_project_id}, ds=${body.stitch_design_system_id} by ${body.bot_id}`,
        );
      ...
      case 'clarification-ready': {
        ...
        console.log(
          `[Callback] Clarification ready: ${qaSections.length} Q&A sections for ${project.project_name} by ${body.bot_id}`,
        );
      ...
      case 'design-reference-analysis': {
        ...
        console.log(
          `[Callback] Design reference analysis saved: section=${refSection.section_id} by ${body.bot_id}`,
        );
      ...
      case 'design-prototype': {
        ...
        if (!hasStitchAttempt) {
          console.warn(
            `[Callback] design-prototype fallback without any stitch attempt for ${body.service_id} by ${body.bot_id}`,
          );
        }
        ...
        console.log(
          `[Callback] Design prototype saved: material=${protoMaterial.material_id}, section=${screenSection.section_id} by ${body.bot_id}${!hasStitchAttempt ? ' (FALLBACK)' : ''}`,
        );
      ...
      case 'feature-spec-ready': {
        ...
        console.log(`[Callback] Feature spec ready: ${body.feature_id} by ${body.bot_id}`);
      ...
      case 'feature-work-complete': {
        ...
        if (isLifecyclePipeline) {
          ...
          console.log(
            `[Callback] Feature work complete (pipeline → in-test): ${body.feature_id} by ${body.bot_id}`,
          );
        ...
        console.log(`[Callback] Feature work complete: ${body.feature_id} by ${body.bot_id}`);
      ...
      case 'feature-test-complete': {
        ...
        if (body.test_passed) {
          ...
          console.log(
            `[Callback] Feature test passed → active: ${body.feature_id} by ${body.bot_id}`,
          );
          ...
          console.log(
            `[Callback] Feature test failed → in-dev: ${body.feature_id} by ${body.bot_id}`,
          );
      ...
      case 'feature-discovery-complete': {
        ...
        console.log(
          `[Callback] Feature discovery complete: ${body.session_id}, ${body.candidates.length} candidates by ${body.bot_id}`,
        );
      ...
      case 'feature-spec-enriched': {
        ...
        console.log(`[Callback] Feature spec enriched: ${body.feature_id} by ${body.bot_id}`);
      ...
      case 'feature-conversation-complete': {
        ...
        console.log(
          `[Callback] Feature conversation complete: ${body.session_id}, ${body.features?.length ?? 0} features by ${body.bot_id}`,
        );
      ...
      case 'deploy-verification': {
        ...
        console.log(
          `[Callback] Deploy verification for ${body.service_id} infra-phase ${body.infra_phase}: ${verification.overall_status} by ${body.bot_id}`,
        );
      ...
      case 'sandbox-section-submit': {
        ...
        console.log(
          `[Callback] Sandbox section "${body.section_key}" (Phase ${body.phase}) submitted by ${body.bot_id}`,
        );
        return NextResponse.json({ ok: true, section });
      }
