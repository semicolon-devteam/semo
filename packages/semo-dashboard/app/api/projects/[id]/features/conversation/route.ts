...
import { dispatchFeatureConversation } from '@/lib/service-bot';
...
async function handleUpdateFeatures(body: Record<string, unknown>) {
  ...
  const session = await updateConversationSession(session_id, {
    features: features as FeatureConversationSession['features'],
  });
