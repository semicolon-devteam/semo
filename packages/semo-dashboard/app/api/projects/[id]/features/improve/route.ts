  ...
  try {
    ...
    if (!project?.service_domain) {
      return NextResponse.json(
        { error: 'Project not found or no service_domain' },
        { status: 404 },
      );
    }
    ...
    if (!repoPath) {
      return NextResponse.json(
        { error: `KB에 '${project.service_domain}/repo' 엔트리가 없습니다. 먼저 등록해주세요.` },
        { status: 400 },
      );
    }
    ...
    if (!repoMatch) {
      return NextResponse.json(
        { error: `repo 형식이 올바르지 않습니다: ${repoPath}` },
        { status: 400 },
      );
    }
    ...
    if (!ghResponse.ok) {
      ...
      return NextResponse.json(
        { error: `GitHub Issue 생성 실패: ${ghResponse.status}` },
        { status: 502 },
      );
    }
