...
import { createMaterial, listMaterials, createSectionsFromMapping } from '@/lib/service';
import type { ServicePhaseMapping } from '@/types';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const material = await createMaterial({
      service_id: id,
      content,
      phase_mapping: phase_mapping as ServicePhaseMapping[] | undefined,
    });
    ...
    if (phase_mapping && Array.isArray(phase_mapping)) {
      const highCoverage = phase_mapping.filter(
        (m: ServicePhaseMapping) => m.coverage >= 0.3 && m.sections.length > 0,
      );
