...
export async function POST(
  ...
  try {
    ...
    const constraints = extractListFromSections(approved0, ['constraint', 'non-negotiable', 'accessibility']);

    // PRD에서 기능 목록, 유저 스토리 추출
    const features = extractListFromSections(approved2, ['feature', 'function', 'requirement']);
    const userFlows = extractListFromSections(approved1.concat(approved2), ['flow', 'journey', 'story', 'scenario']);
  ...
  for (const section of sections) {
    ...
    for (const line of lines) {
      ...
      if (/^[-*]\s|^\d+\.\s|^- \[/.test(trimmed)) {
        ...
        if (text.length > 5 && keywords.some((k) => section.section_key.toLowerCase().includes(k) || text.toLowerCase().includes(k))) {
          items.push(text);
        }
      }
    }
