...
import { updateFeature } from '@/lib/gfp';
...
export async function PATCH(
  ...
  try {
    ...
    if (errors.length > 0) {
      return NextResponse.json({ error: 'Spec validation failed', details: errors }, { status: 400 });
    }
...
async function handleGenerate(serviceId: string, featureId: string) {
  const { getProject } = await import('@/lib/gfp');
  const { dispatchSpecEnrichment } = await import('@/lib/gfp-bot');
...
async function handleTestResult(featureId: string, body: Record<string, unknown>) {
  const { scenario_id, result, notes } = body as { scenario_id: string; result: 'pass' | 'fail' | 'skip'; notes?: string };
