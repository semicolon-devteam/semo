...
async function handleStartScan(serviceId: string, body: Record<string, unknown>) {
  ...
  if (!sourceUrl) {
    return NextResponse.json({ error: 'URL이 필요합니다. body.url을 지정하거나 KB에 service-url을 등록하세요.' }, { status: 400 });
  }
  ...
  dispatchFeatureDiscovery(session.session_id, sourceUrl, project.service_domain)
    .catch((err: unknown) => {
...
async function handleConfirm(serviceId: string, body: Record<string, unknown>) {
  ...
  if (!session_id || !features || !Array.isArray(features)) {
    return NextResponse.json({ error: 'session_id and features array are required' }, { status: 400 });
  }
