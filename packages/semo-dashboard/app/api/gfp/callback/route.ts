  ...
  try {
    ...
    switch (body.type) {
      case 'section-regeneration': {
        ...
        if (project) {
          const slackCtx = await resolveGfpSlackContext(section.service_id);
          sendGfpSectionPendingReviewSlack({
            projectName: project.project_name,
            gfpId: section.service_id,
      ...
      case 'stitch-export': {
        ...
        if (stitchProject) {
          const stitchSlackCtx = await resolveGfpSlackContext(body.service_id);
      ...
      case 'clarification-ready': {
        ...
        // Send Q&A to Slack
        const slackCtx = await resolveGfpSlackContext(body.service_id);
      ...
      case 'design-prototype': {
        ...
        if (!hasStitchAttempt) {
          ...
          if (protoProject && isFirstFallback) {
            const protoSlackCtx = await resolveGfpSlackContext(body.service_id);
      ...
      case 'feature-work-complete': {
        ...
        if (workProject) {
          const workSlackCtx = await resolveGfpSlackContext(completedFeature.service_id);
