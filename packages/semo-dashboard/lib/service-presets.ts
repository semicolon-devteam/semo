...
export function shouldSkipCc(metadata: Record<string, unknown>, phase: number): boolean {
  const presetConfig = metadata?.preset_config as ServicePresetConfig | undefined;
 ...
 */
export function resolveBotHint(template: string, presetConfig: ServicePresetConfig): string {
 ...
 */
export function getBotHintForPhase(
