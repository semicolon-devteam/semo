...
function InfoCard({ label, value, href }: { label: string; value: string | null; href?: string }) {
  ...
  return (
    <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-4">
      ...
      {href ? (
        <a href={href} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-400 hover:underline break-all">
          {value}
        </a>
      ) : (
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <InfoCard label="GitHub Repo" value={kb.repo} href={kb.repo?.startsWith('http') ? kb.repo : kb.repo ? `https://github.com/${kb.repo}` : undefined} />
      ...
      {/* Description */}
      {description && (
        <MarkdownSection title="서비스 설명" content={description} />
      )}
