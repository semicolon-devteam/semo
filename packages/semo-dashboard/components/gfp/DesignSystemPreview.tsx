...
export default function DesignSystemPreview({ sections }: DesignSystemPreviewProps) {
  ...
  const colors = useMemo(
    () => (colorContent ? parseColors(colorContent) : []),
    [colorContent]
  );
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-3 space-y-2">
      ...
      <PreviewPanel
        title="색상"
        status={colorSection?.status}
        defaultOpen
      >
        {colors.length > 0 ? (
          <ColorPreview groups={colors} />
        ) : (
          <Placeholder />
        )}
      </PreviewPanel>
      ...
      <PreviewPanel title="타이포그래피" status={typoSection?.status}>
        {typography.length > 0 ? (
          <TypographyPreview entries={typography} />
        ) : (
          <Placeholder />
        )}
      </PreviewPanel>
      ...
      <PreviewPanel title="여백" status={spacingSection?.status}>
        {spacing.length > 0 ? (
          <SpacingPreview entries={spacing} />
        ) : (
          <Placeholder />
        )}
      </PreviewPanel>
...
function ColorPreview({ groups }: { groups: ColorGroup[] }) {
  return (
    <div className="space-y-2 max-h-[200px] overflow-y-auto">
      {groups.map((group) => (
        <div key={group.name}>
          <p className="text-[10px] text-gray-500 dark:text-gray-400 capitalize mb-1">{group.name}</p>
