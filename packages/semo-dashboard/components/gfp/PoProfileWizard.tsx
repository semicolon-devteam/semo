...
const QUESTIONS: QuestionDef<string>[] = [
  {
    ...
    options: [
      { value: 'non-technical', label: '비개발자', description: '코딩 경험이 없거나 거의 없어요', icon: '🌱' },
      { value: 'basic', label: '기본', description: 'HTML/CSS 정도는 읽을 수 있어요', icon: '🌿' },
      { value: 'intermediate', label: '중급', description: '프레임워크를 사용해본 적 있어요', icon: '🌳' },
  ...
  },
  {
    ...
    options: [
      ...
      { value: 'medium', label: '적당히 신경 쓰고 싶어요', description: '주요 스타일은 직접 고르고 싶어요', icon: '🎨' },
      { value: 'high', label: '디테일까지 직접 조율', description: '자간, 행간, 그리드까지 신경 써요', icon: '✨' },
    ],
  },
  {
    key: 'domain_area',
    ...
    options: [
      { value: 'business', label: '비즈니스/마케팅', description: '시장, 수익 모델, 고객 확보', icon: '📊' },
      { value: 'engineering', label: '엔지니어링', description: '아키텍처, 코드, 인프라', icon: '⚙️' },
      { value: 'design', label: '디자인', description: 'UI/UX, 브랜딩, 비주얼', icon: '🎯' },
      { value: 'product', label: '프로덕트 기획', description: '기능 정의, 우선순위, 로드맵', icon: '🗺️' },
    ],
  },
  {
    ...
    options: [
      { value: 'concise', label: '핵심만 간결하게', description: '결론 위주로, 배경 설명은 최소화', icon: '🎯' },
      { value: 'detailed', label: '맥락과 함께 상세하게', description: '왜 이렇게 했는지 설명도 함께', icon: '📖' },
    ],
  },
  {
    key: 'decision_style',
    ...
    options: [
      { value: 'options', label: '여러 옵션 보여줘', description: '비교하고 직접 골라볼게요', icon: '🔀' },
      { value: 'recommendation', label: '추천안 하나만 줘', description: '전문가 판단에 맡길게요', icon: '👆' },
    ],
  },
];
...
const LABELS: Record<string, Record<string, string>> = {
  tech_level: { 'non-technical': '비개발자', basic: '기본', intermediate: '중급', advanced: '고급' },
  design_sensitivity: { low: '실용 우선', medium: '적당히 꼼꼼', high: '디테일 장인' },
  domain_area: { business: '비즈니스', engineering: '엔지니어링', design: '디자인', product: '프로덕트' },
          ...
          <div className="space-y-3 mb-6">
            {QUESTIONS.map((q) => {
              ...
              return (
                ...
                >
                  <span className="text-sm text-gray-600 dark:text-gray-400">{q.title.replace('?', '').replace('는', '')}</span>
