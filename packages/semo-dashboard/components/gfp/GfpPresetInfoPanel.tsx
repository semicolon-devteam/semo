...
import type { GfpInfraConfig, GfpPresetConfig } from '@/types';
...
export default function GfpPresetInfoPanel({ metadata }: GfpPresetInfoPanelProps) {
  ...
  const config = metadata?.preset_config as GfpPresetConfig | undefined;
...
function InfraReadyPanel({ infra }: { infra: GfpInfraConfig }) {
  return (
      ...
      <div className="space-y-2 text-sm">
        ...
        {infra.dns_configured !== undefined && (
          <div>
            ...
            <p className="text-gray-700 dark:text-gray-300">{infra.dns_configured ? 'Configured' : 'Pending'}</p>
          </div>
        )}
