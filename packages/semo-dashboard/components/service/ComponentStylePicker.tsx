...
const STYLE_PRESETS: StylePreset[] = [
  ...
  },
  {
    ...
    buttonClass:
      'px-5 py-2.5 text-white text-sm font-medium rounded-xl bg-white/20 backdrop-blur-md border border-white/30',
    cardClass: 'bg-white/10 backdrop-blur-lg border border-white/20 p-5 rounded-2xl shadow-lg',
    inputClass:
      'bg-white/10 backdrop-blur-sm border border-white/20 px-4 py-2.5 text-sm rounded-xl text-white placeholder-white/50',
  },
  {
    key: 'soft-shadow',
    label: '소프트 섀도',
    ...
    buttonClass:
      'px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg shadow-md shadow-blue-600/30 hover:shadow-lg',
...
export default function ComponentStylePicker({
  ...
  const handleSelect = useCallback(
    ...
    [sectionId, serviceId, onStyleSelected],
  );
  ...
  return (
    <div className="space-y-6">
      {/* Button Styles */}
      <div>
        <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">버튼 스타일</h4>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {STYLE_PRESETS.map((preset) => (
            ...
            >
              ...
              {/* Preview area */}
              <div
                className={`min-h-[80px] flex items-center justify-center rounded-lg mb-3 ${
                  preset.key === 'glass-premium'
                    ? 'bg-gradient-to-br from-gray-800 to-gray-900 p-4'
                    : 'bg-gray-50 dark:bg-gray-800 p-4'
                }`}
              >
                <span className={preset.buttonClass}>확인</span>
              </div>
      ...
      {/* Card Styles */}
      <div>
        <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">카드 스타일</h4>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {STYLE_PRESETS.map((preset) => (
            <div
              key={`card-${preset.key}`}
      ...
      {/* Input Styles */}
      <div>
        <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">인풋 스타일</h4>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {STYLE_PRESETS.map((preset) => (
            <div
      ...
      {/* Status */}
      {saving && (
        <p className="text-xs text-gray-400 dark:text-gray-500 text-center">선택 저장 중...</p>
      )}
      {selected && !saving && (
