...
describe('buildInternalRoster', () => {
  it('OPENCLAW_BOTS → internal 엔트리(dispatchable), 보조메타 결합', () => {
    ...
    expect(roster[0]).toMatchObject({ audience: 'internal', displayName: 'SemiClaw', dispatchable: true });
...
describe('buildRosterContextBlock', () => {
  it('internal 엔트리만 routing 대상으로 노출(customer 제외)', () => {
    const r: ResolvedRoster = {
      ...
      entries: [
        ...
        { audience: 'customer', agentSlug: 'jumuni', displayName: '주문이', installId: 'i1', dispatchable: false },
      ],
    };
  ...
  it('internal 없으면 빈 문자열', () => {
    const r: ResolvedRoster = {
      ...
      entries: [{ audience: 'customer', agentSlug: 'jumuni', displayName: '주문이', installId: 'i1', dispatchable: false }],
    };
...
describe('loadSemiRoster', () => {
  const deps = { openclawBots: new Set(['workclaw', 'semiclaw']), displayMeta: parseBotIdsDisplayMeta(KB) };
  ...
  it('tenantSlug 있음 → customer(installs, dispatchable=false, install축 식별자)', async () => {
    const rows: CustomerInstallRow[] = [
      { agent_slug: 'jumuni', display_name: '주문이', role_label: '주문 응대', install_id: 'inst-1', instance_name: '주문이#1', persona_slug: 'jumuni', pinned_version: 2 },
    ];
    ...
    const e = r.entries[0];
    expect(e).toMatchObject({ audience: 'customer', agentSlug: 'jumuni', installId: 'inst-1', dispatchable: false, personaSlug: 'jumuni', pinnedVersion: 2 });
    expect('botId' in e).toBe(false); // botId 단일축 아님
  });

  it('customer installs 비면 source=empty', async () => {
    const r = await loadSemiRoster({ tenantSlug: 't' }, { ...deps, queryCustomerInstalls: async () => [] });
