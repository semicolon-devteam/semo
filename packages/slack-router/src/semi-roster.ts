...
export const FALLBACK_ROSTER: InternalRosterEntry[] = [
  {
    ...
    roleLabel: 'PM/오케스트레이터',
    dispatchable: true,
  },
  {
    audience: 'internal',
    botId: 'planclaw',
    displayName: 'PlanClaw',
    roleLabel: '기획',
    dispatchable: true,
  },
  {
...
export function buildRosterContextBlock(r: ResolvedRoster): string {
  ...
  lines.push(
    '이 목록 밖의 이름으로는 ROUTE 하지 않는다. 적합한 대상이 없으면 직접 답하거나 한 가지 질문으로 구체화한다.',
  );
  return lines.join('\n');
}
