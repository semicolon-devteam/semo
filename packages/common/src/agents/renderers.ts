...
export class OpenClawAgentRenderer implements AgentRenderer {
  ...
  render(spec: AgentSpec, context: RenderContext = {}): RenderedAgentArtifact[] {
    ...
    const configPatch = {
      ...
      // delegation-guard before_dispatch hook 활성 (KB decision/delegation-enforcement-2-layer-2026-05-14)
      hooks: {
        internal: {
          load: {
            extraDirs: ['./plugins'],
    ...
    return [
      ...
      jsonArtifact(
        ...
        path.join(openclawRoot, 'plugins', 'delegation-guard', 'metadata.json'),
        {
          ...
          export: 'default',
          description:
            'KB delegation 룰 강제 — 봇이 자기 키워드 외 메시지 받을 시 escalate 안내 후 agent invoke 차단',
          botId: spec.botId,
        },
      ),
      {
        target: this.target,
        path: path.join(openclawRoot, 'plugins', 'delegation-guard', 'handler.js'),
        format: 'javascript',
        content: openclawDelegationGuardHandler(spec.botId),
      },
    ];
  }
}
...
function openclawDelegationGuardHandler(botId: string): string {
  return `// OpenClaw before_dispatch hook — KB delegation 룰 강제 (auto-generated by OpenClawAgentRenderer)
// 봇 id: ${botId}
// SoT 템플릿: scripts/openclaw-hooks/delegation-guard/handler.js
