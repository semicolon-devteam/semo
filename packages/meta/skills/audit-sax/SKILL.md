---
name: audit-sax
description: 전체 SAX 패키지 통합 품질 감사. Use when (1) 전체 패키지 품질 점검, (2) Agent/Skill/Command 표준 준수 검토, (3) 비효율적 구조 탐지, (4) 정기 감사 수행.
tools: [Bash, Read, Grep, Glob]
---

> **🔔 시스템 메시지**: 이 Skill이 호출되면 `[SAX] Skill: audit-sax 호출 - {감사 범위}` 시스템 메시지를 첫 줄에 출력하세요.

# audit-sax Skill

> 전체 SAX 패키지 Agent/Skill/Command 통합 품질 감사

## Purpose

모든 SAX 패키지를 일괄 검토하여 표준 위반, 비효율적 구조, 품질 문제를 탐지합니다.
...
```bash
# 1. 전체 패키지 스캔
for pkg in sax-meta sax-next sax-backend sax-po sax-qa sax-core sax-pm; do
  echo "=== $pkg ==="
...
wc -l sax-*/skills/*/SKILL.md | awk '$1 > 100'
...
```markdown
[SAX] Skill: audit-sax 호출 - {범위}

[SAX] Audit: Agent 검사 중... ({n}개)
[SAX] Audit: Skill 검사 중... ({n}개)
[SAX] Audit: Command 검사 중... ({n}개)

[SAX] Skill: audit-sax 완료 (🔴 {n}건, 🟡 {n}건, 🟢 {n}건)
```
