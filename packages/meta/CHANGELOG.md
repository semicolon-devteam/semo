...
All notable changes to sax-meta will be documented in this file.
...
### Added
- `/SEMO:*` 커맨드 추가 (SAX 커맨드와 병행)
  - `/SEMO:help`, `/SEMO:slack`, `/SEMO:update`, `/SEMO:feedback`, `/SEMO:audit`, `/SEMO:health`
- `scripts/sax-to-semo-migrate.sh` - SAX → SEMO 마이그레이션 스크립트
...
### Added
- `docs/SEMO_NAMING_CONVENTION.md` - SAX → SEMO 리브랜딩 용어 매핑 및 전환 가이드
...
### Added
- Initial sax-meta package setup
