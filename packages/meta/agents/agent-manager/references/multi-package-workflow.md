...
| 접두사 | 대상 | 예시 |
|--------|------|------|
| `[po]` | sax-po 패키지만 | `[po] 새 Agent 추가해줘` |
| `[next]` | sax-next 패키지만 | `[next] Agent 수정해줘` |
| `[core]` | sax-core 패키지만 | `[core] 원칙 추가해줘` |
| `[meta]` | sax-meta 패키지만 | `[meta] Agent 삭제해줘` |
...
| 패키지 | agents/ 경로 |
|--------|-------------|
| sax-po | `sax-po/agents/` |
| sax-next | `sax-next/agents/` |
| sax-meta | `sax-meta/agents/` |
| sax-core | `sax-core/` (agents 없음) |
...
```markdown
[SAX] Agent: agent-manager 호출 - Multi-Package Agent 생성
...
[SAX] Agent: agent-manager 호출 - Agent 생성

✅ sax-po/agents/{agent-name}.md 생성 완료
...
✅ sax-next/agents/{agent-name}.md 생성 완료
...
**생성된 파일**:
- `sax-po/agents/{agent-name}.md`
- `sax-next/agents/{agent-name}.md`
```
...
**실패**:
- sax-next: ❌ 에러 (이미 동일 이름의 Agent 존재)
...
| 패키지 | 차이점 |
|--------|--------|
| sax-po | `--repo semicolon-devteam/sax-po` |
| sax-next | `--repo semicolon-devteam/sax-next` |
| sax-meta | `--repo semicolon-devteam/sax-meta` |
