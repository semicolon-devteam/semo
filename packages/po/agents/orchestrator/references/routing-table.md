...
> SEMO-PO Orchestrator의 라우팅 결정 테이블

## Routing Decision Table

| User Intent | Route To | Detection Keywords |
|-------------|----------|-------------------|
| 도움 요청 | `skill:semo-help` | "/SEMO:help", "도움말", "뭘 해야 하지" |
| SEMO init 커밋 | `sax-init` 프로세스 | "SEMO init", "SEMO 설치 커밋", "SEMO init 커밋해줘" |
| 피드백 | `skill:feedback` | "/SEMO:feedback", "피드백", "피드백해줘", "버그 신고" |
| SEMO 동작 오류 지적 | `skill:feedback` | "SEMO가 왜", "SEMO 동작이", "[SEMO] 메시지가" |
| 온보딩 요청 | `onboarding-master` | "/SEMO:onboarding", "처음", "신규", "온보딩" |
| 환경 검증 | `skill:health-check` | "/SEMO:health-check", "환경 확인", "도구 확인" |
| SEMO 업데이트/검증 | `version-updater` | "SEMO 업데이트", "최신버전", "SEMO 동기화", "패키지 업데이트", "업데이트 검증", "업데이트가 제대로", "설치 확인", "심링크 확인", "제대로 설치됐는지" |
