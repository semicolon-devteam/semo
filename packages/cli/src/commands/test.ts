...
async function executeTestSuite(
  ...
  return new Promise<RunResult>((resolve) => {
    ...
    child.on("close", async (code) => {
      ...
      const status =
        code !== 0 || fail > 0 ? "failed" : cases.length === 0 ? "error" : "passed";
      ...
      try {
        ...
        console.error(
          chalk.red(`  DB 기록 오류: ${err.message}`)
        );
      }
    ...
    .action(async (suiteArg: string | undefined, options: any) => {
      ...
      try {
        ...
        if (suiteArg) {
          ...
          if (rows.length === 0) {
            console.error(
              chalk.red(`스위트 '${suiteArg}'를 찾을 수 없습니다.`)
            );
            const allSuites = await pool.query("SELECT suite_id FROM semo.test_suites");
            console.log(
              chalk.gray(
                `등록된 스위트: ${allSuites.rows.map((r: any) => r.suite_id).join(", ")}`
              )
            );
            return;
          }
        ...
        for (const suite of suites) {
          ...
          try {
            ...
            if (result.status === "passed") {
              ...
              spinner.warn(
                `${suite.suite_id}: ${chalk.yellow("ERROR")} (출력 파싱 실패)`
              );
            }
          } catch (err: any) {
            spinner.fail(
              `${suite.suite_id}: ${chalk.red("ERROR")} — ${err.message}`
            );
            results.push({
              runId: "error",
        ...
        if (options.notify && !allPassed) {
          ...
          for (const fr of failedResults) {
            ...
            if (sent) {
              console.log(
                chalk.gray(`  Slack 알림 전송: ${fr.suiteId}`)
              );
            }
          }
        }
