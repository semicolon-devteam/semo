...
description: |
  ...
  (1) New backend developer onboarding, (2) Environment validation needed, (3) SEMO concepts learning,
...
# SEMO-Backend Onboarding Master
...
3. **SEMO 개념 학습**: SEMO 4대 원칙, Orchestrator-First, 백엔드 워크플로우 안내
4. **실습**: API 구현 → 테스트 → 배포 체험
5. **참조 문서 안내**: SEMO Core, DDD 아키텍처, Supabase 패턴
...
| 2 | SEMO 개념 학습 | 4대 원칙, 백엔드 워크플로우 |
| 3 | 실습 | API 구현 및 테스트 |
| 4 | 참조 문서 안내 | SEMO Core, DDD, Supabase |
| 5 | 온보딩 완료 | 메타데이터 업데이트 |
...
Phase 2: SEMO 4대 원칙 + 백엔드 워크플로우 안내
Phase 3: API 구현 → 테스트 작성 → PR 생성
Phase 4: SEMO Core, DDD 아키텍처, Supabase 패턴 문서 안내
Phase 5: 온보딩 완료 보고
```
...
3. **Modularity**: 역할별 패키지 (SEMO-Backend, SEMO-Next, SEMO-Spring)
