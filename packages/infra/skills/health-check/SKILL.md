...
> **🔔 시스템 메시지**: 이 Skill이 호출되면 `[SEMO] Skill: health-check 호출 - 인프라 환경 검증` 시스템 메시지를 첫 줄에 출력하세요.
...
신규/기존 인프라 엔지니어의 개발 환경을 자동으로 검증하여 SEMO 사용 준비 상태를 확인합니다.
...
# SEMO 패키지 설치 상태 확인
ls -la .claude/semo-infra/ 2>/dev/null && echo "✅ semo-infra 설치됨"
...
if [ "$POSITION" != "infra" ]; then
  echo "❌ position 값이 'infra'가 아님: $POSITION"
fi
```
...
| 항목 | 검증 방법 |
|------|----------|
| 패키지 디렉토리 | `.claude/semo-core/`, `.claude/semo-infra/` 존재 확인 |
| CLAUDE.md 심링크 | `.claude/CLAUDE.md` → `semo-infra/CLAUDE.md` |
| agents 심링크 | `.claude/agents` → `semo-infra/agents` |
| skills 심링크 | `.claude/skills` → `semo-infra/skills` |
| commands 심링크 | `.claude/commands/SEMO` → `../semo-infra/commands` |
...
✅ SEMO 메타데이터: 존재
✅ SEMO 패키지: semo-core, semo-infra 설치됨
✅ 심링크: 정상
...
[SEMO] Reference: 인프라 환경 검증 (도구/인증/Docker) 완료
```
