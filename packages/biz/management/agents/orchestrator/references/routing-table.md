...
| **정기회의록**, 회의록 생성, N월 N/N 회의록, 회의록 만들어줘 | **generate-meeting-minutes** | 기간 정보 기반 회의록 생성 | **1 (높음)** |
| "회의록" AND (정리 OR 요약), STT 분석 | summarize-meeting | STT 스크립트 + 회의록 URL 제공 시 | 2 (낮음) |
| 의사결정 로그, decision log | create-decision-log | 결정사항 기록 요청 시 | - |
| 테스트 요청, 테스트요청, QA 요청, QA에게 알림 | request-test | Issue 기반 테스트 요청 Slack 전송 | - |

### 회의록 관련 스킬 라우팅 규칙

> **🔴 summarize-meeting은 복합 조건 필수**: "회의록" 단독으로는 트리거되지 않음

```text
"회의록" 키워드 감지
    │
    ├─ "정기회의록" / "N월 N/N 회의록" / "회의록 만들어줘" / "회의록 생성"
    │   └→ generate-meeting-minutes (새 회의록 Discussion 생성)
    │
    └─ ("회의록" AND ("정리" OR "요약")) / "STT 분석" / 회의록 URL 제공됨
        └→ summarize-meeting (기존 회의록에 내용 작성)

⚠️ 주의: "정기회의록 만들어줘"는 generate-meeting-minutes로 라우팅!
   - "회의록 정리해줘" → summarize-meeting ✅
   - "회의록 요약해줘" → summarize-meeting ✅
   - "정기회의록 만들어줘" → generate-meeting-minutes ✅
   - "12월 4/4 회의록" → generate-meeting-minutes ✅
