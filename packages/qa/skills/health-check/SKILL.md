...
> **🔔 시스템 메시지**: 이 Skill이 호출되면 `[SEMO] Skill: health-check 호출 - QA 환경 검증` 시스템 메시지를 첫 줄에 출력하세요.
...
신규/기존 QA 팀원의 개발 환경을 자동으로 검증하여 SEMO 사용 준비 상태를 확인합니다.
...
# SEMO 패키지 설치 상태 확인
ls -la .claude/semo-qa/ 2>/dev/null && echo "✅ semo-qa 설치됨"
...
| 항목 | 검증 방법 |
|------|----------|
| 패키지 디렉토리 | `.claude/semo-core/`, `.claude/semo-qa/` 존재 확인 |
| CLAUDE.md 심링크 | `.claude/CLAUDE.md` → `semo-qa/CLAUDE.md` |
| agents 심링크 | `.claude/agents` → `semo-qa/agents` |
| skills 심링크 | `.claude/skills` → `semo-qa/skills` |
| commands 심링크 | `.claude/commands/SEMO` → `../semo-qa/commands` |
...
✅ SEMO 메타데이터: 존재
✅ SEMO 패키지: semo-core, semo-qa 설치됨
✅ 심링크: 정상
...
[SEMO] Reference: QA 환경 검증 (도구/인증/테스트 도구) 완료
```
