...
name: onboarding
description: |
  SEMO 통합 온보딩 프로세스 (공통 Skill). Use when (1) /SEMO:onboarding 커맨드,
  (2) "처음이에요", "신규", "온보딩" 키워드, (3) 신규 팀원 온보딩 필요 시.
  환경 진단 → 조직 참여 → SEMO 학습 → 패키지별 실습 → 완료 메타데이터 저장.
tools: [Read, Bash, Glob, Grep, WebFetch]
model: inherit
---

> **시스템 메시지**: `[SEMO] Skill: onboarding 호출`

# SEMO Onboarding Skill

> 신규 팀원을 위한 SEMO 통합 온보딩 프로세스 (SEMO Core 공통 Skill)

## Purpose

모든 SEMO 패키지에서 공통으로 사용하는 온보딩 Skill입니다.
...
| Phase | 내용 | 호출 대상 |
|-------|------|----------|
| 0 | 환경 진단 | skill:health-check (또는 /SEMO:health) |
| 1 | 조직 참여 확인 | Slack, GitHub Org |
| 2 | SEMO 개념 학습 | PRINCIPLES.md, MESSAGE_RULES.md |
...
1. **Transparency**: 모든 AI 작업은 `[SEMO] ...` 메시지로 표시됩니다.
2. **Orchestrator-First**: 모든 요청은 Orchestrator가 먼저 분석합니다.
3. **Modularity**: 역할별 패키지로 분리됩니다 (PO, Next, QA 등).
4. **Hierarchy**: SEMO Core의 원칙을 각 패키지가 상속합니다.

### 개발자 워크플로우

```text
1. 이슈 할당: "cm-{project}#{issue_number} 할당받았어요"
2. SEMO 분석: 이슈 복잡도 분석 → 작업 계획 제안
3. 개발 진행: SEMO가 코드 작성 지원
4. PR 생성: SEMO가 PR 템플릿 자동 생성
5. 완료 보고: Slack 알림
```
...
[SEMO] Skill: onboarding-{package} 호출 (Phase 3)
