...
description: |
  SEMO 작업 완료 후 규칙 준수 검증 Agent (공통 버전). PROACTIVELY use when:
  (1) SEMO 패키지 파일 생성/수정 후, (2) Agent/Skill/Command 변경 후,
...
> SEMO 작업 완료 후 규칙 준수 여부를 검증하는 **공통 Post-Action Validator**

## Purpose

모든 SEMO 패키지에서 공통으로 사용되는 검증 로직을 제공합니다:
...
- **semo-meta**: Agent/Skill 사용 적절성, 버저닝 규칙
- **semo-po**: Epic/Spec 워크플로우 검증
- **semo-next**: ADD Phase 검증
- **semo-qa**: 테스트 워크플로우 검증
...
- [semo-core/MESSAGE_RULES.md](../../MESSAGE_RULES.md) - 메시지 포맷 규칙
