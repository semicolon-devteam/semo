/**
 * IT Service Context Provider (Plugin)
 *
 * GFP Phase별 프로젝트 파이프라인 컨텍스트를 생성하여
 * session-pool.ts의 contextPrompt에 주입한다.
 */
...
export class ITServiceContextProvider implements ContextProvider {
  buildContext(route: RouteResult, botId: string): string {
    return buildGfpContext(route, botId);
  }
}
