...
### 🔴 이슈 작업 시작 라우팅 체인 (NON-NEGOTIABLE)

> **⚠️ 이슈 기반 작업 시작 시 반드시 아래 체인을 따릅니다.**
...
**핵심 규칙:**
- SDD 선택 시 `advisor` → `skill:start-task` → (사용자 선택) → `skill:spec` 또는 `skill:implement`
- Fast-track 선택 시 `advisor` → `skill:fast-track`
- ❌ `start-task` 없이 바로 `spec-master` 호출 금지
- ❌ 수동 브랜치/PR 명령어 안내 금지
...
- `skill:start-task` - 작업 시작 자동화 (Draft 전환, 브랜치, Draft PR)
