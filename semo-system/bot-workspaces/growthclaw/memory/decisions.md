...
## 크론 잡/설정 변경 지시 처리 (NON-NEGOTIABLE, 2026-03-14, Reus 지시, SemiClaw 전파)
**필수 규칙:**
1. Reus가 **크론 잡 수정을 지시**하면 → `cron` tool로 **실제 크론 잡 설정을 변경**해야 함
...
## 후속 보고 보장 원칙 (NON-NEGOTIABLE, 2026-03-15, Reus 지시, SemiClaw 전파)
**반복 문제:** 봇들이 "확인하고 보고할게"라고 하고 세션 끊기면 실제 보고 안 함

**필수 규칙:**
1. **시간이 걸리는 작업 후 보고가 필요하면 → cron job 등록**으로 기술적 보장
   - 결과 확인 + 보고를 크론으로 스케줄링
2. **기술적으로 보장할 수 없으면 "보고할게"라고 약속하지 말 것**
   - 대신 사용자에게 확인 방법 안내
3. **CI/CD 등 자동화 가능한 건 → Slack Webhook 알림** 등 봇 의존 없는 방식 권장

**요약:** 보고 약속 = 보장된 메커니즘 필수. 보장 못 하면 약속 금지.
