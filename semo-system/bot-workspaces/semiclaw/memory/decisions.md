...
### 2026-03-07: PR 리뷰어 지정 — AI/봇/에이전트 관련은 Reus
...
## "보고할게" 후 사라지는 문제 — 후속 보고 보장 원칙 (3/15 Reus 지적)
- 봇들이 "확인하고 보고할게"라고 하고 세션 종료 후 실제 보고 안 하는 패턴 반복
- **원칙**: 시간이 걸리는 작업 후 보고가 필요하면 반드시 cron job 등록해서 결과 확인 + 보고를 기술적으로 보장할 것
- 보장할 수 없으면 "보고할게"라고 약속하지 말 것
- CI/CD 등은 워크플로우 자체에 Slack 알림 step 추가 권장 (봇 의존 제거)
- InfraClaw 포함 전체 봇 해당
...
4. ✅ Slack 멘션 허용 (2026-03-07 변경) — 라벨+폴링도 병행
...
## 크론 잡 수정 지시 → 실제 크론 잡 설정 수정 필수 (2026-03-14, Reus 지시)
- **봇이 크론 잡 수정 지시를 받으면 실제 cron tool로 크론 잡 설정을 변경해야 함**
- ❌ decisions.md, memory.md 등 메모리 파일만 수정하고 끝내면 안 됨
- 채널 ID/프로젝트 정보 모르면 SemiClaw에게 질문
- 배경: ReviewClaw가 PR 리뷰 폴링 보고 채널 변경 지시에 decisions.md만 수정하고 cron job은 안 고침
...
- GitHub 라벨+폴링 인계 방식 유지 + Slack 멘션도 허용 (2026-03-07 변경)
