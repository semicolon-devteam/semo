 ...
 * Usage: npx ts-node --esm scripts/generate-summaries.ts
...
const EXP_DESCRIPTIONS: Record<string, Record<string, string>> = {
  ko: {
...
const HORIZON_DESCRIPTIONS: Record<string, Record<string, string>> = {
...
function getToneGuide(riskLabel: string, lang: string): string {
...
async function main() {
  ...
  const occTaskMap = new Map<string, typeof occTasks>();
  ...
  // Fetch occupation_task_experience
  ...
  // Group by occupation_id + experience_level
  const expMap = new Map<string, typeof expData>();
  ...
  for (let i = 0; i < occupations.length; i++) {
    ...
    // Build context for this occupation
    ...
    // Build exp-level risk context
    const expContext: string[] = [];
    ...
    // Determine which combos need generation
    const needed: { exp: string; horizon: string; lang: string }[] = [];
    ...
    // Generate in batches by language (12 combos per lang)
    for (const lang of LANGS) {
      ...
      const combosText = langNeeded.map(n => {
        ...
        if (expTasks.length > 0) {
          ...
          riskRate = expTasks.reduce((s, t) => s + ((t as any)[field] || t.ai_replacement_rate || 0), 0) / expTasks.length;
        } else {
...
${combosText}`;
      ...
      try {
        ...
        const summaries: { key: string; text: string }[] = [];
        ...
        console.log(`  ${lang}: parsed ${summaries.length} summaries`);
      } catch (err) {
