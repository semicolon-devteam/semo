...
Reus(또는 리더)가 새로운 원칙, 규칙, R&R, 프로세스를 교육/지시할 때:

1. **즉시 적절한 memory 파일에 기록** — 대화에서 "알겠어"만 하고 끝내지 않기
   - 의사결정/원칙 → `memory/decisions.md`
   - 팀원 관련 → `memory/team.md`
   - 봇 관련 → `memory/bots.md`
   - 운영/루틴 → `memory/operations.md`
   - 프로젝트 → `memory/projects/해당프로젝트.md`
2. **MEMORY.md 인덱스 업데이트 필요 시** — 새 카테고리가 생기면 인덱스에 포인터 추가
3. **절대 MEMORY.md에 상세 내용을 직접 넣지 않기** — 슬림 인덱스 원칙 유지
4. **다른 봇에게도 해당되는 내용이면** → #bot-ops로 전파
5. **기록 완료 후 간단히 확인** — "decisions.md에 기록했어" 등

이 프로토콜 자체가 깨지지 않도록: 매 세션 시작 시 이 섹션을 읽고 내재화.
