...
## 크론 잡/설정 변경 지시 처리 (NON-NEGOTIABLE, 2026-03-14 Reus 지시)

**핵심 원칙: 크론 잡 수정 지시 = 실제 cron tool 설정 변경 필수**

### 필수 규칙
1. ✅ **Reus가 크론 잡 수정을 지시하면 → `cron` tool로 실제 크론 잡 설정 변경**
2. ❌ **`decisions.md`, `memory.md` 등 메모리 파일만 수정하고 끝내면 안 됨**
3. ✅ **채널 ID/프로젝트 매핑 등 정보를 모르면 → SemiClaw(`<@U0ADGB42N79>`)에게 질문**

### 지시 유형 구분
| 지시 유형 | 작업 대상 | 예시 |
|---------|----------|------|
| 크론 잡 수정 | `cron` tool로 실제 설정 변경 | "크론 잡 스케줄 바꿔", "보고 채널 변경해" |
| 규칙 추가 | `decisions.md`/`memory.md` 기록 | "이 원칙 기억해", "규칙 추가해" |
| 교육 수용 | `decisions.md`/`memory.md` 기록 | "앞으로 이렇게 해" (설정이 아닌 행동 원칙) |

### 예시

**올바른 처리**:
- Reus: "크론 잡 보고를 각 프로젝트 채널로 바꿔"
- 봇: `cron` tool로 실제 job 설정의 delivery.channel 변경

**잘못된 처리** (금지):
- Reus: "크론 잡 보고를 각 프로젝트 채널로 바꿔"
- 봇: `decisions.md`에만 "앞으로 각 프로젝트 채널로 보고한다" 기록 → ❌ 실제 설정 안 바뀜

### 정보 부족 시
- 채널 ID, 프로젝트→채널 매핑 등 모르면 → SemiClaw에게 `[bot:info-req]` 형식으로 질문
- 추측하지 말고 정확한 정보 확보 후 변경
...
## Epic 생성 워크플로우 (2026-03-08 Reus 지시, 2026-03-08 위반 사례 재강조)

**핵심 원칙: Epic Issue 먼저 생성 → 문서 작성 → 유저 검수 → 승인 후 `bot:spec-ready`**

### ⚠️ 필수 게이트: 유저 검수 없이 라벨 전환 절대 금지
- **라벨 변경 = 유저 승인 후에만 가능**
- **봇이 자의적으로 `bot:spec-ready` 라벨을 다는 것은 엄격히 금지**
- 미완성/잘못된 스펙이 다음 단계로 넘어가면 전체 워크플로우 오염

### 올바른 순서
1. **Epic Issue 생성** (GitHub)
   - 라벨 없이 생성 (또는 기본 라벨만)
   - Epic 제목 및 간단한 개요만 포함
   - 상태: 검수 대기
   
2. **Epic 문서 작성** (상세 기획)
   - 프로젝트 개요, 타겟 유저, MVP 스코프, 타임라인 등
   - **GitHub Issue 본문에 직접 작성** (로컬 파일 X)
   
3. **⭐ 유저 검수 요청 (필수 게이트)**
   - Epic 문서 내용 Slack/이슈 코멘트로 공유
   - Reus/PO에게 검수 요청
   - **이 단계를 건너뛰면 안 됨**
   
4. **승인 후 라벨 전환**
   - Reus/PO 승인 확인 후 → `bot:spec-ready` 라벨 추가
   - WorkClaw이 폴링으로 감지 → 작업 시작

### 잘못된 워크플로우 (금지)
- ❌ Epic 문서 먼저 작성 → Issue 생성 + 바로 `bot:spec-ready`
- ❌ **유저 검수 없이 `bot:spec-ready` 라벨 추가 ← 심각한 위반**
- ❌ Epic Issue 없이 문서만 작성

### PlanClaw 적용 사항
- Epic 요청 시: Issue 먼저 → 문서 → **검수 요청 → 승인 확인** → 승인 후 라벨
- Feature 이슈도 동일 규칙 적용 (Issue → 기획서 → 검수 → 승인 후 `bot:spec-ready`)
- **검수 대기 중인 이슈에는 `bot:*` 라벨 붙이지 말 것**

### 예외
- 간단한 Task 이슈 (1-2문장 스펙): 검수 없이 바로 `bot:spec-ready` 가능
- Epic/Feature 등 큰 단위는 반드시 검수 필요

### 기획서 작성 위치 (2026-03-08 Reus 지시)
- ❌ **로컬 파일 경로 금지**: `/Users/reus/...` 같은 로컬 경로는 Reus가 접근 불가
- ✅ **GitHub Issue 본문에 직접 작성**: Epic/Feature 기획서는 Issue body에 전체 내용 복사
- ✅ **예외 (UI 포함 프론트 작업)**: 화면설계서는 `semicolon-devteam/docs` 레포 → GitHub Pages 링크 사용
- **원칙**: 팀원이 접근 가능한 위치만 사용 (로컬 파일 X, GitHub O)
...
## 후속 보고 보장 원칙 (2026-03-15, Reus 지시, SemiClaw 전파)

**핵심 문제: 봇들이 "확인하고 보고할게"라고 하고 세션 끊기면 실제 보고 안 하는 패턴**

### 필수 규칙

1. **시간이 걸리는 작업 후 보고가 필요하면 → cron job 등록**
   - 기술적으로 보고를 보장하는 메커니즘 구축
   - 예: "1시간 후 PR 상태 확인해서 보고" → cron job으로 등록
   
2. **기술적으로 보장할 수 없으면 "보고할게"라고 약속하지 말 것**
   - 대신 사용자에게 확인 방법을 안내
   - 예: "이슈 #123에서 확인 가능합니다" (보고 약속 X)
   
3. **CI/CD 등 자동화 가능한 건 → Slack Webhook 알림 등 봇 의존 없는 방식 권장**
   - GitHub Actions → Slack Webhook 직접 알림
   - 봇의 세션 상태에 의존하지 않는 구조

### PlanClaw 적용 사항
- 기획서 작성 후 "나중에 보고"가 필요한 경우 → cron job으로 등록
- 단순 "작업 완료 안내"는 즉시 보고 or 확인 방법 안내 (약속 X)
- 장기 작업 추적이 필요하면 GitHub Issue 링크 공유 (직접 보고 약속하지 말 것)
