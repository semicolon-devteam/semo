...
## 🚨 할루시네이션 재발 방지 규칙 (2026-03-08 AXOracle 사건)

**사건**: InfraClaw(나)가 AXOracle이 "아직 Vercel"이라고 확신 있게 답변 → 실제로는 전날 OCI로 이전 완료 + Vercel 프로젝트 삭제됨. GrowthClaw, WorkClaw까지 잘못된 전제를 수용해서 봇 3개가 헛바퀴.
...
- ❌ "다음에 하지" → ✅ "지금 바로"
- 특히 InfraClaw: 배포 타겟 변경(Vercel→OCI 등)은 MEMORY.md 프로젝트 정보 섹션 즉시 업데이트 필수
...
- 다른 봇이 전달한 전제("Vercel 배포 중" 등)를 그대로 신뢰 금지
...
### AXOracle 배포 상태 할루시네이션 (2026-03-08)
**사건**: InfraClaw(나)가 AXOracle이 "아직 Vercel"이라고 확신 있게 답변 → 실제로는 전날(2026-03-07) OCI로 이전 완료 + Vercel 프로젝트 삭제됨
- **영향**: GrowthClaw, WorkClaw까지 잘못된 전제를 수용해서 봇 3개가 헛바퀴
- **근본 원인**:
  1. OCI 이전 작업 완료 후 MEMORY.md 미업데이트
  2. 메모리만 믿고 실제 상태 확인 안 함 (vercel ls, kubectl, curl 등)
  3. 봇 간 인계 시 전달받은 전제를 독립 검증 없이 수용

**교훈**:
1. **중대 작업 후 메모리 즉시 업데이트 필수**
   - 배포 타겟 변경(Vercel→OCI), 인프라 변경 등 → 해당 세션에서 MEMORY.md 반드시 업데이트
   - "다음에 하지" 절대 금지

2. **상태 질문엔 실제 확인 우선 (Trust but Verify)**
   - 배포/인프라 상태 질문 → 메모리 의존 금지
   - 실제 확인: `kubectl get pods`, `vercel ls`, `curl -I`, OCI 콘솔, `gh api` 등
   - 메모리는 참고용, 실제 상태가 source of truth

3. **봇 간 인계 시 독립 검증**
   - 다른 봇이 전달한 전제("Vercel 배포 중") 그대로 신뢰 금지
   - 본인이 직접 확인 가능한 수단으로 검증 후 작업 착수

**재발 방지 (2026-03-08 즉시 적용)**:
- MEMORY.md에 할루시네이션 재발 방지 규칙 섹션 추가
- AGENTS.md에 Rule 1/2/3 명시
- 매 세션 시작 시 SOUL.md, AGENTS.md, MEMORY.md 필독 확인

**앞으로**:
- 프로젝트 배포 상태 질문 시 반드시 실제 확인 (kubectl, vercel, curl 등)
- 메모리 정보와 실제 상태가 불일치하면 → 실제 상태 우선 + MEMORY.md 즉시 수정
- 중대 작업 완료 시 MEMORY.md 업데이트 체크리스트 추가 검토
