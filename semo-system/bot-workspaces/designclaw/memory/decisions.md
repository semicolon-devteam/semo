...
## 크론 잡/설정 변경 지시 처리 (2026-03-14, Reus 지시, NON-NEGOTIABLE)

**절대 규칙:**
1. Reus가 크론 잡 수정을 지시하면 → **실제 cron tool로 크론 잡 설정을 변경**해야 함
...
**배경**: ReviewClaw이 크론 잡 수정 지시를 받고 cron 설정을 실제로 변경하지 않고 decisions.md만 수정한 사례 → Reus 재교육
...
### DesignClaw 적용 사항
- 디자인 스펙 완료 후 → `bot:spec-ready` 라벨만 부착 (WorkClaw 멘션도 허용)
- UI 리뷰 요청 받으면 → 코멘트 작성 + 라벨 전환 (멘션도 허용)
...
## 후속 보고 보장 원칙 (2026-03-15, Reus 지시, NON-NEGOTIABLE)

### 배경
봇들이 "확인하고 보고할게"라고 하고 세션 끊기면 실제 보고 안 하는 패턴 반복

### 필수 규칙
1. **시간이 걸리는 작업 후 보고가 필요하면 → cron job 등록**
   - 결과 확인 + 보고를 기술적으로 보장할 것
...
### DesignClaw 적용 사항
- 디자인 리뷰 요청 시 즉시 처리 가능하면 바로 리뷰
- 시간이 걸리면 → cron job으로 보고 보장 또는 확인 방법 안내
- "확인하고 보고할게" 같은 약속 금지
