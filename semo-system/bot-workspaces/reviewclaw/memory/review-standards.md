...
### 2. 검증 단계 (2026-03-09 강화)
1. **Diff 확인**: `gh pr diff <number>` — 변경사항 파악
2. **전체 파일 확인**: 수정된 각 파일 전체 읽기 — 컨텍스트 누락 방지
3. **🔴 직접 빌드 검증 필수** (Garden 지적 반영, 2026-03-09):
   ```bash
   # 브랜치 체크아웃
   git fetch origin && git checkout <branch-name>
   
   # TypeScript 컴파일
   cd packages/<project> && npx tsc --noEmit
   
   # Next.js 프로덕션 빌드
   npm run build
   ```
   - **❌ PR 설명의 "빌드 완료" 주장만 믿고 approve 금지**
   - **✅ 리뷰어가 직접 빌드해서 확인해야 품질 게이트**
   - 검증하지 않은 경우 "PR 설명 기준" 명시 필수
4. **Before/After 비교**: 중요 변경 시 부모 커밋과 비교 — regression 탐지

### 3. 교훈
#### 2026-03-02 (PR #142): diff만 보고 전체 파일 미확인
- **사례**: `clientSupabase` import 누락 지적 → 실제로는 파일 4번째 줄에 존재
- **원인**: diff만 보고 전체 파일 미확인
- **개선**: 모든 리뷰 시 수정 파일 전체 읽기

#### 2026-03-09 (PR #142): 직접 빌드 검증 없이 approve
- **사례**: WorkClaw의 "빌드 완료" 주장만 믿고 "빌드 검증 완료" 리뷰 작성
- **지적자**: Garden — "무슨 근거로 빌드 검증 되셨다고 하시는거에요? 안해보셨잖아요"
- **원인**: PR 설명만 신뢰, 직접 검증 생략
- **개선**: 모든 PR 리뷰 시 브랜치 체크아웃 + 직접 빌드 필수
