...
## 🔴 작업 방식 선택 후 자동 라우팅 (NON-NEGOTIABLE)

> **⚠️ SDD 또는 Fast-track 선택 후, 반드시 해당 스킬을 호출합니다. 수동 안내 금지.**

### 선택 → 스킬 라우팅 테이블

| 사용자 선택 | 라우팅 대상 | 동작 |
|------------|------------|------|
| **A. SDD** | `skill:start-task` | Draft 전환 + 상태 변경 + 브랜치 + Draft PR + 다음 단계 선택 |
| **B. Fast-track** | `skill:fast-track` | 간소화된 수정 프로세스 |

### SDD 선택 시 출력 (필수)

```markdown
[SEMO] Advisor: SDD 방식 선택 확인

→ `skill:start-task` 호출하여 작업 환경을 세팅합니다.

[SEMO] Skill: start-task 호출
```

### Fast-track 선택 시 출력 (필수)

```markdown
[SEMO] Advisor: Fast-track 방식 선택 확인

→ `skill:fast-track` 호출하여 간소화된 수정을 진행합니다.

[SEMO] Skill: fast-track 호출
```

### 🔴 금지 사항

- ❌ SDD 선택 후 수동으로 브랜치/PR 명령어 안내
- ❌ `start-task` 스킬 호출 없이 직접 `spec-master` 호출
- ❌ 작업 환경 세팅 단계 생략
...
skill:start-task 완료
    ↓
사용자에게 다음 단계 선택지 제시 (start-task가 제시)
    ├─ "A. 스펙부터 시작" → skill:spec
    ├─ "B. 바로 구현" → skill:implement
    └─ "C. Fast-track" → skill:fast-track
```
