...
> docs 레포지토리 및 SEMO 패키지 문서 인덱스

## semo-core 문서

| 파일 | 내용 | 중복 검토 범위 |
|------|------|---------------|
| `PRINCIPLES.md` | SEMO 핵심 원칙 | 모든 원칙 정의 |
| `MESSAGE_RULES.md` | SEMO 메시지 포맷 규칙 | 메시지 포맷, 출력 규칙 |
...
| `semo-architect` | 패키지 설계 | 구조, 설계, 아키텍처 |
...
| 체크 항목 | 비교 대상 | 위반 조건 |
|----------|----------|----------|
| Orchestrator 규칙 | `semo-core/PRINCIPLES.md` 섹션 3 | 동일/유사 규칙 존재 |
| Agent 규칙 | `semo-core/PRINCIPLES.md` 섹션 4 | 동일/유사 규칙 존재 |
| Skill 규칙 | `semo-core/PRINCIPLES.md` 섹션 5 | 동일/유사 규칙 존재 |
| 메시지 포맷 | `semo-core/MESSAGE_RULES.md` | 동일 포맷 정의 |
| 패키지 구조 | `semo-core/PACKAGING.md` | 동일 구조 정의 |
...
| 정보 유형 | SoT 위치 | 참조 방식 |
|----------|---------|----------|
| SEMO 핵심 원칙 | `semo-core/PRINCIPLES.md` | `@import` 또는 링크 참조 |
| 메시지 규칙 | `semo-core/MESSAGE_RULES.md` | `@import` 또는 링크 참조 |
| 팀 규칙 | `semo-core/TEAM_RULES.md` | `@import` 또는 링크 참조 |
...
```bash
# 1. 유사 문서 검색
grep -r "{keyword}" semo-core/ agents/ skills/
