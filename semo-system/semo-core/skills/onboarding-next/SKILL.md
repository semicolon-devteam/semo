...
# 확인 사항
- mcp__supabase__list_tables 호출 확인
- 테이블 목록 출력 확인
```
...
# 확인 사항
- mcp__supabase__get_table_schema 호출
- 컬럼명, 타입, nullable 정보 확인
```

#### 5-3. Repository 패턴 예시

```markdown
# core-supabase 참조 방법
> "posts Repository 예시 코드 보여줘"

# 확인 사항
- fetch-supabase-example Skill 호출
- RPC 함수 호출 패턴 확인
- 타입 assertion 패턴 확인 (as unknown as Type)
```
...
```text
✅ 필수 규칙:
1. 항상 RPC 함수 사용 (raw SQL 금지)
2. 파라미터는 p_ 접두사 (p_user_id, p_content)
3. 타입 assertion: as unknown as Type
4. 에러 체크 후 data 사용
5. core-supabase 패턴 우선 확인

❌ 금지 사항:
- supabase.from().select() 직접 호출
- 커스텀 RPC 임의 생성
- Optional 파라미터에 undefined 직접 전달
```
