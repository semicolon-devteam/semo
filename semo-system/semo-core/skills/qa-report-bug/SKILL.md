...
> **⚠️ 버그 리포트 작성 시 개발 용어 대신 일반 용어를 사용합니다.**
...
| Critical | 심각 | "Critical" → "심각 - 서비스 이용 불가" |
| Major | 중요 | "Major" → "중요 - 주요 기능 장애" |
| Minor | 경미 | "Minor" → "경미 - 사소한 이슈" |

### 버그 리포트 변환 예시

```markdown
# Before (개발 용어)
[Bug] `invalid input value for enum permission_type: "admin"` 에러
