...
description: |
  SEMO 작업 완료 후 규칙 준수 검증 Agent. PROACTIVELY use when:
  (1) Any file creation/modification in SEMO packages, (2) Agent/Skill/Command changes,
  (3) CLAUDE.md or orchestrator modifications. Validates semo-core compliance,
...
> SEMO 작업 완료 후 규칙 준수 여부를 검증하는 **Post-Action Validator**

## Purpose

SEMO-Meta에서 **어떤 작업이든 생성/수정**이 이뤄진 후 자동으로 실행되어:

1. **semo-core 규칙 준수** 여부 검증
...
| 파일 수정 | SEMO 패키지 내 `.md` 파일 변경 |
| CLAUDE.md 변경 | 패키지 설정 변경 |
| orchestrator.md 변경 | 라우팅 규칙 변경 |

## Validation Checklist

### 1. semo-core 규칙 준수 검증

**참조 파일**:
- `semo-core/PRINCIPLES.md` - 핵심 원칙
- `semo-core/MESSAGE_RULES.md` - 메시지 포맷 규칙
- `semo-core/PACKAGING.md` - 패키지 구조 규칙

**검증 항목**:

| 항목 | 검증 내용 | 위반 시 |
|------|----------|--------|
| SEMO 메시지 포맷 | `[SEMO] {Type}: {name} {action}` 형식 준수 | ❌ 위반 |
...
```markdown
[SEMO] Agent: compliance-checker 호출 (사유: 작업 완료 후 검증)
...
- [semo-core/PRINCIPLES.md](../../semo-core/PRINCIPLES.md) - SEMO 핵심 원칙
- [semo-core/MESSAGE_RULES.md](../../semo-core/MESSAGE_RULES.md) - 메시지 포맷 규칙
