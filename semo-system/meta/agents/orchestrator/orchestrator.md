...
## 🔴 Post-Action Triggers (자동 실행)

> **작업 완료 후 자동으로 트리거되는 스킬/에이전트**

### 파일 변경 감지 시 자동 트리거

| 변경 대상 | 자동 트리거 | 설명 |
|----------|------------|------|
| `agents/**/*.md` | `version-manager` | Agent 파일 수정 시 버저닝 제안 |
| `skills/**/*.md` | `version-manager` | Skill 파일 수정 시 버저닝 제안 |
| `commands/**/*.md` | `version-manager` | Command 파일 수정 시 버저닝 제안 |
| `packages/cli/**` | `deploy-npm` | CLI 수정 시 npm 배포 제안 |
| `CLAUDE.md` | `compliance-checker` | 설정 변경 시 규칙 검증 |
...
[파일 변경 감지]
    ↓
Agent/Skill/Command 변경?
    ├─ Yes → [SEMO] Post-Action: version-manager 호출
    │         "버전 업데이트가 필요합니다. 진행할까요?"
    └─ No → compliance-checker만 실행
```

### 버전 업데이트 제안 포맷

```markdown
[SEMO] Post-Action: 버전 업데이트 필요

📦 **변경된 패키지**:
| 패키지 | 현재 버전 | 권장 버전 | 변경 파일 |
|--------|----------|----------|----------|
| {package} | {current} | {recommended} | {files} |

버전 업데이트를 진행할까요? (yes/no)
```
