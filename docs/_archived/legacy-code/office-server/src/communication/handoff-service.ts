...
export class HandoffService extends EventEmitter {
  ...
  getPendingForRole(officeId: string, role: AgentRole): HandoffRequest[] {
    return Array.from(this.pendingHandoffs.values()).filter(
      (h) => h.officeId === officeId && h.toRole === role && h.status === 'pending',
    );
  }
   ...
   */
  async getHistory(agentId: string, options: GetHistoryOptions = {}): Promise<HandoffRequest[]> {
  ...
  async cancelHandoff(handoffId: string): Promise<void> {
    ...
    // Update database
    if (this.supabase) {
      await this.supabase.from('handoff_requests').delete().eq('id', handoffId);
    }
