...
> 모든 Agent는 팀 공용 GitHub 계정으로 작업합니다. `gh` CLI가 해당 계정으로 로그인되어 있어야 합니다.
...
Agent별로 커밋 author를 분리하여 기여 추적이 가능합니다.

```bash
# Agent별 git config 설정 (Worktree 생성 시 자동 설정)
git config user.name "FE-Agent (최프론트)"
git config user.email "fe-agent@semo.ai"
```

### Author 매핑

| Role       | Git Author Name              | Git Author Email            |
|------------|------------------------------|-----------------------------|
| PO         | PO-Agent (박기획)            | `po-agent@semo.ai`          |
| PM         | PM-Agent (김피엠)            | `pm-agent@semo.ai`          |
| Architect  | Architect-Agent (이설계)     | `architect-agent@semo.ai`   |
| FE         | FE-Agent (최프론트)          | `fe-agent@semo.ai`          |
| BE         | BE-Agent (정백엔드)          | `be-agent@semo.ai`          |
| QA         | QA-Agent (한큐에이)          | `qa-agent@semo.ai`          |
| DevOps     | DevOps-Agent (윤데봅스)      | `devops-agent@semo.ai`      |
| Decomposer | Decomposer-Agent (김분해)    | `decomposer-agent@semo.ai`  |

### 구현

```typescript
// Worktree 생성 시 git config 자동 설정
async function configureGitAuthor(worktreePath: string, persona: AgentPersona): Promise<void> {
  const authorName = `${persona.role}-Agent (${persona.name})`;
  const authorEmail = `${persona.role.toLowerCase()}-agent@semo.ai`;

  await exec(`git config user.name "${authorName}"`, { cwd: worktreePath });
  await exec(`git config user.email "${authorEmail}"`, { cwd: worktreePath });
}
```
